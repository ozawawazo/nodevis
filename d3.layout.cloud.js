...
(function(exports) {
	...
	var size = [256, 256],
	    ...
	    event = d3.dispatch("word", "end", "end2"),
	    timer = null,
	    cloud = {};
	...
	while (++di < n) {
	    d = data[di];
	    c.save();
	    c.font = d.style + " " + d.weight + " " + ~~((d.size + 1) / ratio) + "px " + d.font;//配置
	    var w = c.measureText(d.text + "m").width * ratio,
		h = d.size << 1;
	    if (d.rotate) {
		var sr = Math.sin(d.rotate * cloudRadians),    
		    cr = Math.cos(d.rotate * cloudRadians),    //傾き関連
		    wcr = w * cr,
		    wsr = w * sr,
		    hcr = h * cr,
		    hsr = h * sr;
	    ...
	    if (y + h >= ch) break;
	    c.translate((x + (w >> 1)) / ratio, (y + (h >> 1)) / ratio);//ある条件下のものは画面外にはじき出す、1ビットを右にシフト
	    if (d.rotate) c.rotate(d.rotate * cloudRadians);
	    c.fillText(d.text, 0, 0);//指定された text が指定された座標 (x, y) にレンダリングされます
